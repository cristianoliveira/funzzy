...
## Timeout layout increment evidence

- Source locations and move proof: `.tmp/reports/04-09-26/task-0182-timeout-layout.md`.
- Commit `4d9bfc4` moves `timeout_config_tests` directly after `parse_duration`; test path, assertions, comments, visibility, duration/error contracts, and parser behavior are unchanged.
- Config tests 127, architecture guard 8, and fresh watcher gen221 pass. This was the final layout cluster.

## Closure evidence

- Full criterion-by-criterion matrix and baseline/final move-aware inventory: `.tmp/reports/04-09-26/task-0182-closure.md`.
- Final source order leaves no production implementation after the broad test tail; dedicated clusters are paired with owning production or validation sections. No public paths, visibility, parser results, errors, comments, or test paths changed.
- Plan move validation: `kanban.py done TASK-0182` moved this file from `plans/todo/` to `plans/done/` and set status to `done`.
