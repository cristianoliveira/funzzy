...
fzz control run TARGET --wait --timeout 60s
```
...
- [ ] Tests first cover status, list, run, run-and-wait, unavailable socket, malformed response, timeout, failed run, and superseded generation.
...
- [ ] `run TARGET` returns scheduled generation; `--wait` exits according to that exact generation's terminal outcome.
