...
fn missing_generation_and_task_are_actionable_errors() {
    ...
    assert!(
        stdout.contains(&format!("retained generations are {generation}")),
        "actionable range: {stdout}"
    );
    ...
    assert!(
        stdout.contains("retained tasks:") && stdout.contains("failing task"),
        "actionable tasks: {stdout}"
    );
}
...
fn watcher_restart_clears_all_retained_output() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("nothing is retained"), "stdout: {stdout}");
    let _ = replacement;
}
