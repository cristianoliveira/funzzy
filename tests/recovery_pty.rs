...
fn run_with_answer(config: &Path, answer: &[u8]) -> (std::process::ExitStatus, String) {
...
fn default_decline_keeps_failure_and_does_not_mutate() {
    ...
    let (status, output) = run_with_answer(&config, b"n\n");
