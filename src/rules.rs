            ...
            for item in items {
                ...
                match item {
                    Yaml::Hash(_) if item["tasks"] != Yaml::BadValue => {
                        ...
                        match parse_hash_format(item, config_dir.clone()) {
...
fn parse_hash_format(yaml: &Yaml, config_dir: Option<String>) -> errors::Result<Vec<Rules>> {
    ...
    for task_yaml in tasks_array {
        match rule_from_with_common(task_yaml, &common_rules, config_dir.clone()) {
            Ok(rule) => rules.push(rule),
            Err(err) => return Err(err),
        }
    }
  ...
  - name: build
    ...
    fn it_maintains_backward_compatibility_with_all_formats() {
        ...
        - name: task1
          ...
          change: 'src/**'
        ";
        let rules1 = from_yaml(classic, None).expect("Failed to parse classic format");
        ...
        tasks:
          - name: task2
            run: echo 'single'
        ";
        let rules2 = from_yaml(single_group, None).expect("Failed to parse single group format");
